...
#### Adding readable parameter values
...
The CSV file used in this example includes the following columns:
...
| `Partner name`         | The name of the user's organization, used to look up the corresponding ID in the reporting partner table                                          |
...
1.  **Retrieve `Partner` reference Based on `Partner name`:**
    -   The script maps the `Partner name` column to the corresponding partner IDs in the reporting partner table.
    -   If any partner names do not match the existing table, the script will raise an error.
2.  **Assign Roles, Parameters, and Grants:**
    -   Using the CSV data, the script assigns roles to users and dynamically applies optional grants and parameters as defined in the file.
...
```{r eval=FALSE, include=TRUE}
databaseId <- "<the database id>" # Replace with your database ID
reportingPartnerReferenceFormId = "<the partner form id>" # Replace with your partner form ID
users <- readr::read_csv("UsersToModify.csv") # load the CSV template
```

#### Step 1: Retrieve `Partner` reference based on `Partner name` column

We need to first get the partner ID to fill our parameter `Partner` for each user with the correct reference.

```{r eval=FALSE, include=TRUE}
# Fetch partner names and IDs from the reporting partner form
partners <- getRecords(reportingPartnerReferenceFormId) |> collect()

allPartnerNames <- na.omit(unique(partners[["Partner name"]]))

# Check for unmatched partner names in the CSV.
unmatchedPartners <- users[["Partner name"]][
  !is.na(users[["Partner name"]])&!users[["Partner name"]] %in% allPartnerNames
]


if (length(unmatchedPartners) > 0) {
  stop("The following values in `Partner name` do not have a corresponding _id in the Reporting Partners form: ", paste(unique(unmatchedPartners), collapse = ", "), ". Please correct the names and try again.")
}

# Map `Partner name` to corresponding partner IDs
partnerIdMap <- deframe(partners[, c("Partner name", "_id")])

# Add missing IDs to the `p:Partners` column as a parameter reference
users <- users |>
  mutate(
    `p:Partner` = coalesce(
      `p:Partner`,
      {
        partnerId <- unname(partnerIdMap[`Partner name`])  # Lookup partner ID and drop names
        ifelse(
          !is.na(partnerId) & !is.null(partnerId), 
          reference(formId = reportingPartnerReferenceFormId, recordId = partnerId), 
          NA_character_
        )
      }
    )
...
Now we have the partner ID for each user's `Partner` parameter.

#### Step 2: Identify columns for optional grants (`g:`) and parameters (`p:`).
...
#### Step 3: Assign roles, parameters and grants
